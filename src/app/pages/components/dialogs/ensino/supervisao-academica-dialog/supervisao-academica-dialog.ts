...
import { IsupervisaoAcademica } from 'src/app/core/interfaces/ensino.interface';
import { SupervisaoServiceName } from './supervisaoName.service';

@Component({
  selector: 'app-supervisao-academica-dialog',
  templateUrl: './supervisao-academica-dialog.html',
  styleUrls: ['./supervisao-academica-dialog.scss']
})
export class SupervisaoAcademicaDialogComponent  implements OnInit {
  displayedColumns: string[] = ['tagName', 'sector', 'edit', 'delete'];

  atividadeLetiva: IsupervisaoAcademica | undefined;
  ensinoForm = false;
  isCreate = false;

  ensinoData!: MatTableDataSource<IsupervisaoAcademica>;
  ensinoSize = 0;
  ...
  constructor(
    ...
    private crudService: CrudService<IsupervisaoAcademica>,
    private sharedDataService: SupervisaoServiceName,
  ...
  async ngOnInit() {
    ...
    try {
      const result = await this.crudService.getAllEnsino('supervisao_academica',this.data.nomeRelatorio).toPromise();
      ...
      if (result) {
        ...
        this.ensinoData = new MatTableDataSource<IsupervisaoAcademica>(result);
  ...
  edit(ensino: IsupervisaoAcademica) {
    this.ensinoForm = true;
    this.isCreate = false;
  ...
  async delete(radoc: IsupervisaoAcademica) {
    ...
    try {
      ...
      if (result && radoc.id !== undefined) { // Verifique se radoc.id está definido
        await this.crudService.delete('supervisao_academica', this.data.nomeRelatorio , this.atividadeLetiva.id).toPromise();

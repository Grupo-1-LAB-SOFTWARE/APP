...
import { IatividadeOrientacao } from 'src/app/core/interfaces/ensino.interface';
import { avOrientacaoServiceName } from './avOrientacaoName.service';
@Component({
  selector: 'app-atividade-orientacao-dialog',
  templateUrl: './atividade-orientacao-dialog.html',
  styleUrls: ['./atividade-orientacao-dialog.scss']
})
export class AtividadeOrientacaoDialogComponent  implements OnInit {
  displayedColumns: string[] = ['tagName', 'sector', 'edit', 'delete'];

  atividadeLetiva: IatividadeOrientacao | undefined;
  ensinoForm = false;
  isCreate = false;

  ensinoData!: MatTableDataSource<IatividadeOrientacao>;
  ensinoSize = 0;
  ...
  constructor(
    ...
    private crudService: CrudService<IatividadeOrientacao>,
    private sharedDataService: avOrientacaoServiceName,
  ...
  async ngOnInit() {
    ...
    try {
      const result = await this.crudService.getAllEnsino('atividade_orientacao_supervisao_preceptoria_tutoria',this.data.nomeRelatorio).toPromise();
      ...
      if (result) {
        ...
        this.ensinoData = new MatTableDataSource<IatividadeOrientacao>(result);
  ...
  async delete(radoc: IatividadeOrientacao) {
    ...
    try {
      ...
      if (result && radoc.id !== undefined) { // Verifique se radoc.id está definido
        await this.crudService.delete('atividade_orientacao_supervisao_preceptoria_tutoria', this.data.nomeRelatorio , this.atividadeLetiva.id).toPromise();

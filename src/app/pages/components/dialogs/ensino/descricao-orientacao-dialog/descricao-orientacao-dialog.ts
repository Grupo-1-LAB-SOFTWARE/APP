...
@Component({
  ...
  templateUrl: './descricao-orientacao-dialog.html',
  styleUrls: ['./descricao-orientacao-dialog.scss']
})
export class DescricaoOrientacaoDialogComponent  implements OnInit {
  displayedColumns: string[] = ['tagName', 'sector', 'edit', 'delete'];

  atividadeLetiva: IdescricaoOrientacao | undefined;
  ensinoForm = false;
  isCreate = false;

  ensinoData!: MatTableDataSource<IdescricaoOrientacao>;
  ensinoSize = 0;
  ...
  async ngOnInit() {
    ...
    try {
      const result = await this.crudService.getAllEnsino('descricao_orientacao_coorientacao_academica',this.data.nomeRelatorio).toPromise();
      ...
      if (result) {
        ...
        this.ensinoData = new MatTableDataSource<IdescricaoOrientacao>(result);
  ...
  edit(ensino: IdescricaoOrientacao) {
    this.ensinoForm = true;
    this.isCreate = false;
  ...
  async delete(radoc: IdescricaoOrientacao) {
    ...
    try {
      ...
      if (result && radoc.id !== undefined) { // Verifique se radoc.id está definido
        await this.crudService.delete('descricao_orientacao_coorientacao_academica', this.data.nomeRelatorio , this.atividadeLetiva.id).toPromise();

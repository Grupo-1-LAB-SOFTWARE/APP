...
import { IchSemanalAtividadesPesquisa } from 'src/app/core/interfaces/pesquisa.interface';
import { chSemanalAtividadePesquisaName } from './chSemanalAtividadePesquisaName.service';

@Component({
  selector: 'app-ch-semanal-atividades-pesquisa-dialog',
  templateUrl: './ch-semanal-atividades-pesquisa-dialog.html',
  styleUrls: ['./ch-semanal-atividades-pesquisa-dialog.scss'],

})
export class CHSemanalAtividadesPesquisaDialogComponent implements OnInit {
  displayedColumns: string[] = ['tagName', 'sector', 'edit', 'delete'];

  atividadePesquisa: IchSemanalAtividadesPesquisa | undefined;
  pesquisaForm = false;
  isCreate = false;

  pesquisaData!: MatTableDataSource<IchSemanalAtividadesPesquisa>;
  pesquisaSize = 0;
  ...
  constructor(
    ...
    private crudService: CrudService<IchSemanalAtividadesPesquisa>,
    private sharedDataService: chSemanalAtividadePesquisaName,
  ...
  async ngOnInit() {
    ...
    try {
      const result = await this.crudService.getAllPesquisa('ch_semanal_atividades_pesquisa',this.data.nomeRelatorio).toPromise();
      ...
      if (result) {
        ...
        this.pesquisaData = new MatTableDataSource<IchSemanalAtividadesPesquisa>(result);
  ...
  edit(pesquisa: IchSemanalAtividadesPesquisa) {
    this.pesquisaForm = true;
    this.isCreate = false;
  ...
  async delete(radoc: IchSemanalAtividadesPesquisa) {
    ...
    try {
      ...
      if (result && radoc.id !== undefined) {
        await this.crudService.delete('ch_semanal_atividades_pesquisa', this.data.nomeRelatorio , this.atividadePesquisa.id).toPromise();

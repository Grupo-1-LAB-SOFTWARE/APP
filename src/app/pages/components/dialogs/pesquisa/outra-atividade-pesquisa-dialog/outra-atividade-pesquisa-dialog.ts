...
import { IoutraAtividadePesquisaProducaoIntelectual } from 'src/app/core/interfaces/pesquisa.interface';
import { outraAtividadePesquisaName } from './outraAtividadePesquisaName.service';

@Component({
  selector: 'app-outra-atividade-pesquisa-dialog',
  templateUrl: './outra-atividade-pesquisa-dialog.html',
  styleUrls: ['./outra-atividade-pesquisa-dialog.scss'],

})
export class OutraAtividadePesquisaDialogComponent implements OnInit {
  displayedColumns: string[] = ['tagName', 'sector', 'edit', 'delete'];

  atividadePesquisa: IoutraAtividadePesquisaProducaoIntelectual | undefined;
  pesquisaForm = false;
  isCreate = false;

  pesquisaData!: MatTableDataSource<IoutraAtividadePesquisaProducaoIntelectual>;
  pesquisaSize = 0;
  ...
  constructor(
    ...
    private crudService: CrudService<IoutraAtividadePesquisaProducaoIntelectual>,
    private sharedDataService: outraAtividadePesquisaName,
  ...
  async ngOnInit() {
    ...
    try {
      const result = await this.crudService.getAllPesquisa('outra_atividade_pesquisa_producao_intelectual',this.data.nomeRelatorio).toPromise();
      ...
      if (result) {
        ...
        this.pesquisaData = new MatTableDataSource<IoutraAtividadePesquisaProducaoIntelectual>(result);
  ...
  edit(pesquisa: IoutraAtividadePesquisaProducaoIntelectual) {
    this.pesquisaForm = true;
    this.isCreate = false;
  ...
  async delete(radoc: IoutraAtividadePesquisaProducaoIntelectual) {
    ...
    try {
      ...
      if (result && radoc.id !== undefined) {
        await this.crudService.delete('outra_atividade_pesquisa_producao_intelectual', this.data.nomeRelatorio , this.atividadePesquisa.id).toPromise();

...
export class ProjetoPesquisaProducaoIntelectualDialogComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    try {
      const result = await this.crudService.getAllPesquisa('projetos_pesquisa_producao_intelectual',this.data.nomeRelatorio).toPromise();
  ...
  async delete(radoc: IprojetoPesquisaProducaoIntelectual) {
    ...
    try {
      ...
      if (result && radoc.id !== undefined) {
        await this.crudService.delete('projetos_pesquisa_producao_intelectual', this.data.nomeRelatorio , this.atividadePesquisa.id).toPromise();

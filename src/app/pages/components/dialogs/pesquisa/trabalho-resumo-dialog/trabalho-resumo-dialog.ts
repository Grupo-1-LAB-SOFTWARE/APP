...
@Component({
  ...
  templateUrl: './trabalho-resumo-dialog.html',
  styleUrls: ['./trabalho-resumo-dialog.scss'],

})
export class TrabalhoCompletoResumoPublicadoDialogComponent implements OnInit {
  displayedColumns: string[] = ['tagName', 'sector', 'edit', 'delete'];

  atividadePesquisa: ItrabalhoCompletoResumoPublicadoApresentadoCongressos | undefined;
  pesquisaForm = false;
  isCreate = false;

  pesquisaData!: MatTableDataSource<ItrabalhoCompletoResumoPublicadoApresentadoCongressos>;
  pesquisaSize = 0;
  ...
  async ngOnInit() {
    ...
    try {
      const result = await this.crudService.getAllPesquisa('trabalho_completo_resumo_publicado_apresentado_congressos',this.data.nomeRelatorio).toPromise();
      ...
      if (result) {
        ...
        this.pesquisaData = new MatTableDataSource<ItrabalhoCompletoResumoPublicadoApresentadoCongressos>(result);
  ...
  edit(pesquisa: ItrabalhoCompletoResumoPublicadoApresentadoCongressos) {
    this.pesquisaForm = true;
    this.isCreate = false;
  ...
  async delete(radoc: ItrabalhoCompletoResumoPublicadoApresentadoCongressos) {
    ...
    try {
      ...
      if (result && radoc.id !== undefined) {
        await this.crudService.delete('trabalho_completo_resumo_publicado_apresentado_congressos', this.data.nomeRelatorio , this.atividadePesquisa.id).toPromise();
